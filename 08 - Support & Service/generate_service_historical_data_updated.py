...
class ServiceDataGenerator:
    def __init__(self, master_data_file=None, crm_data_file=None):
        """Initialize with master and CRM data"""
        ...
        for day in range(DAYS_OF_HISTORY):
            for _ in range(random.randint(5, 15)):
                ...
                ticket = {
                    ...
                    'ticket_number': f"TKT-{self.counters['ticket']-1:06d}",
                    ...
                    'subject': random.choice(['Product defect', 'Technical issue', 'Billing inquiry', 'Installation help']),
                    'description': 'Detailed problem description',
                    'product_id': random.choice(self.products)['product_id'] if self.products else f"PRD-{random.randint(1000, 9999)}",
                    'priority': random.choice(['critical', 'high', 'medium', 'low']),
                    'status': random.choice(['open', 'in_progress', 'waiting', 'resolved']),
                    'assigned_to': random.choice(self.service_agents)['agent_id'] if self.service_agents else 'AGT-000001',
                    'queue_id': random.choice(self.service_queues)['queue_id'] if self.service_queues else 'QUE-000001',
                    'sla_id': random.choice(self.sla_definitions)['sla_id'] if self.sla_definitions else 'SLA-000001',
                    'resolution_date': (ticket_date + timedelta(days=random.randint(1, 5))).strftime('%Y-%m-%d') if random.random() > 0.3 else None,
        ...
        for ticket in self.service_tickets[:500]:  # Comments for first 500 tickets
            ...
            for _ in range(random.randint(1, 3)):
                comment = {
                    ...
                    'commented_by': random.choice(self.service_agents)['agent_id'] if self.service_agents else 'AGT-000001',
                    'comment_date': (datetime.strptime(ticket['created_date'], '%Y-%m-%d') + timedelta(hours=random.randint(1, 48))).strftime('%Y-%m-%d %H:%M:%S'),
        ...
        for ticket in self.service_tickets[:200]:  # Attachments for first 200 tickets
            if random.random() > 0.6:
                attachment = {
                    ...
                    'ticket_id': ticket['ticket_id'],
                    'filename': f"error_log_{random.randint(1, 999)}.txt",
                    ...
                    'attachment_description': 'Screenshot of error or log file',
                    'created_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                }
    ...
    def generate_ticket_escalations(self):
        ...
        for ticket in self.service_tickets[:100]:  # Escalations for 100 tickets
            if random.random() > 0.7:
                escalation = {
                    ...
                    'escalated_to': random.choice(self.service_agents)['agent_id'] if self.service_agents else 'AGT-000001',
                    'escalation_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'escalation_reason': 'SLA breach or customer request',
                    'escalation_level': random.choice(['L2', 'L3']),
                    'resolution_date': (datetime.now() + timedelta(days=random.randint(1, 3))).strftime('%Y-%m-%d') if random.random() > 0.5 else None,
                    'created_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                }
