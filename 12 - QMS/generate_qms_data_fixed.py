...
class QMSDataGenerator:
        ...
        for day in range(days):
            if random.random() < 0.15:
                audit = {
                    ...
                    'followup_date': (current_date + timedelta(days=30)).strftime('%Y-%m-%d') if random.random() > 0.5 else None,
                    'audit_status': random.choice(['completed', 'in_progress']),
                    'audit_report_url': f"https://docs.company.com/audits/audit-{audit_counter:06d}.pdf" if random.random() > 0.4 else None,
                    'closed_date': (current_date + timedelta(days=7)).strftime('%Y-%m-%d') if random.random() > 0.5 else None,
        ...
        for audit in self.data['quality_audits']:
            for idx in range(random.randint(0, audit['total_findings'])):
                finding = {
                    ...
                    'finding_status': random.choice(['open', 'closed']),
                    'closure_date': None,
                    'closure_verified_by': None,
                    'closure_notes': None,
        ...
        for day in range(days):
            if random.random() < 0.1:
                ncr = {
                    ...
                    'product_id': random.choice([p.get('product_id', f'PRD-{random.randint(1, 100):06d}') for p in self.products] + ['PRD-000001']),
                    ...
                    'ncr_status': random.choice(['open', 'contained']),
                    'closed_date': None,
                    'closed_by': None,
    ...
    def generate_capa_headers(self, start_date, days):
        ...
        for day in range(0, days, 7):
            if len(self.data['ncr_headers']) > 0 and random.random() > 0.5:
                ...
                capa = {
                    ...
                    'capa_status': random.choice(['open', 'in_progress']),
                    'closed_date': None,
                    'closed_by': None,
        ...
        for capa in self.data['capa_headers']:
            for seq in range(1, random.randint(2, 4)):
                action = {
                    ...
                    'action_sequence': seq,
                    'action_description': f"Action {seq}",
                    ...
                    'actual_completion_date': None,
                    'action_status': random.choice(['assigned', 'in_progress']),
        ...
        for i in range(min(10, len(self.products))):
            if random.random() > 0.5:
                ppap = {
                    ...
                    'approval_date': (current_date + timedelta(days=random.randint(5, 30))).strftime('%Y-%m-%d') if random.random() > 0.5 else None,
                    'approval_expiry_date': (current_date + timedelta(days=random.randint(180, 730))).strftime('%Y-%m-%d'),
                    'approved_by': 'QM-001' if random.random() > 0.5 else None,
                    ...
                    'psw_signed_by': 'EMP-000001' if random.choice([True, False]) else None,
                    'psw_signed_date': (current_date - timedelta(days=random.randint(1, 30))).strftime('%Y-%m-%d') if random.choice([True, False]) else None,
        ...
        for doc in self.data['quality_documents'][:15]:
            for rev_num in range(1, random.randint(2, 4)):
                revision = {
                    ...
                    'change_description': f"Update",
                    'changes_summary': f"Revision {rev_num} updates",
