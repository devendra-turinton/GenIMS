...
class SupplierPortalDataGenerator:
    def __init__(self, master_data_file=None, erp_data_file=None):
        """Initialize data generator with registry"""
        from pathlib import Path
    ...
    def generate_purchase_requisitions(self):
        ...
        for i in range(30):
            ...
            req = {
                ...
                'requested_by': random.choice(self.employees).get('employee_id', 'EMP-000001') if self.employees else 'EMP-000001',
            ...
            for j in range(num_lines):
                ...
                mat_id = mat.get('material_id', 'MAT-000001') if isinstance(mat, dict) else mat
                line = {
    ...
    def generate_supplier_contracts(self):
        ...
        for i, supplier in enumerate(self.suppliers[:10]):
            ...
            for j, material in enumerate(random.sample(self.materials, num_materials)):
                material_id = material.get('material_id', 'MAT-000001') if isinstance(material, dict) else material
                pricing = {
    ...
    def generate_supplier_invoices(self):
        ...
        for i in range(50):
            ...
            supplier_from_list = random.choice(self.suppliers)
            supplier_id = supplier_from_list.get('supplier_id', 'SUP-000001') if isinstance(supplier_from_list, dict) else supplier_from_list
            po_from_list = random.choice(self.purchase_orders)
            po_id = po_from_list.get('purchase_order_id', 'PO-000001') if isinstance(po_from_list, dict) else po_from_list
        ...
        for i, supplier in enumerate(self.suppliers[:10]):
            ...
            if status in ['approved', 'audit_completed']:
                ...
                audit = {
                    ...
                    'lead_auditor': random.choice(self.employees).get('employee_id', 'EMP-000001') if self.employees else 'EMP-000001',
    ...
    def _generate_supplier_communications(self):
        ...
        communications = []
        for i in range(20):
            sup = random.choice(self.suppliers) if self.suppliers else {'supplier_id': 'SUP-000001'}
            supplier_id = sup.get('supplier_id', 'SUP-000001') if isinstance(sup, dict) else sup
    ...
    def _generate_audit_findings(self):
        ...
        for i in range(25):
            ...
            if status in ['closed', 'pending_verification']:
                actual_closure_date = (datetime.now() + timedelta(days=random.randint(1, 60))).strftime('%Y-%m-%d')
