...
class GenIMSSetup:
    ...
    def log_section(self, title):
        ...
        if "Loading Data" in title:
            logger.info("  🚀 ULTRA-FAST MODE: COPY FROM STDIN + Parallel Table Loading")
            logger.info("  📈 Expected 5-10x speed improvement over INSERT statements")
    ...
    def create_databases(self):
        ...
        logger.info(f"  🔧 Using {max_workers} parallel workers for database operations")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if not success:
                        logger.warning(f"  ⚠ Database operation failed: {db_name}")
                except Exception as e:
                    logger.error(f"  ✗ Database operation error for {db_name}: {e}")
        ...
        if success_count == len(DATABASES):
            logger.info(f"\n✓ Successfully created {success_count} databases in parallel ({elapsed:.1f}s)")
            return True
        else:
            logger.error(f"\n✗ Created {success_count}/{len(DATABASES)} databases ({elapsed:.1f}s)")
            return False
        ...
        try:
            ...
            try:
                ...
                with self.progress_lock:
                    logger.warning(f"  ⚠ Connection termination failed for {db_name}: {str(e)[:80]}")
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_db):
                ...
                try:
                    ...
                    with self.progress_lock:
                        logger.warning(f"  ✗ Schema loading exception for {db_name}: {exc}")
    ...
    def run_single_generator(self, db_name, gen_script, expected_output):
        ...
        try:
            with self.progress_lock:
                logger.info(f"  → Running generator: {gen_script} (timeout: {timeout//60}min)")
            ...
            if result.returncode == 0:
                ...
                with self.progress_lock:
                    ...
                    logger.info(f"    ✓ Generated data for {db_name} ({elapsed:.1f}s)")
                    self.stats['data_generated'] += 1
                return True
            else:
                ...
                if json_path.exists():
                    with self.progress_lock:
                        logger.info(f"    ✓ Generated data for {db_name} (JSON output exists, {elapsed:.1f}s)")
                        self.stats['data_generated'] += 1
                    return True
                else:
                    ...
                    if 'ERROR' in result.stderr or 'FAILED' in result.stderr or 'Exception' in result.stderr:
                        ...
                        with self.progress_lock:
                            logger.info(f"    ✓ Generated data for {db_name} (exit code {result.returncode}, {elapsed:.1f}s)")
                            self.stats['data_generated'] += 1
                        return True
        
        except subprocess.TimeoutExpired:
            with self.progress_lock:
                logger.warning(f"    ✗ Generator timeout ({timeout//60}min): {gen_script}")
    ...
    def run_single_generator_optimized(self, db_name, gen_script, expected_output, complexity, timeout):
        ...
        if not can_run and "Recent output exists" in reason:
            with self.progress_lock:
                logger.info(f"  ⚡ Skipping {db_name}: {reason}")
                self.stats['data_generated'] += 1
            return True
        elif not can_run:
            with self.progress_lock:
                logger.warning(f"  ✗ Cannot run {db_name}: {reason}")
        ...
        try:
            with self.progress_lock:
                ...
                logger.info(f"  → [{complexity_label}] Running: {gen_script}")
            ...
            with self.progress_lock:
                ...
                logger.info(f"      🚀 Parallel mode: batch={batch_size}, COMPLETE data volume guaranteed")
            ...
            if result.returncode == 0:
                ...
                with self.progress_lock:
                    if 'TOTAL' in output:
                        ...
                        for line in lines[-10:]:
                            if 'TOTAL' in line:
                                logger.info(f"      {line.strip()}")
                    logger.info(f"    ✓ [{complexity.upper()}] {db_name} completed ({elapsed:.1f}s)")
                    self.stats['data_generated'] += 1
                return True
            else:
                ...
                if json_exists or 'successfully' in output.lower() or 'completed' in output.lower():
                    with self.progress_lock:
                        logger.info(f"    ✓ [{complexity.upper()}] {db_name} completed with output ({elapsed:.1f}s)")
                        self.stats['data_generated'] += 1
                    return True
                else:
                    # Real failure
                    if 'ERROR' in result.stderr or 'FAILED' in result.stderr or 'Exception' in result.stderr:
                        with self.progress_lock:
                            logger.warning(f"    ✗ [{complexity.upper()}] {db_name} failed: {result.stderr[:150]}")
                            self.stats['errors'].append(f"Generator {gen_script}: {result.stderr[:100]}")
                        return False
                    else:
                        # Unclear failure - treat as success with warning
                        with self.progress_lock:
                            logger.info(f"    ⚠ [{complexity.upper()}] {db_name} completed with warnings ({elapsed:.1f}s)")
                            self.stats['data_generated'] += 1
                        return True
        
        except subprocess.TimeoutExpired:
            with self.progress_lock:
                logger.warning(f"    ✗ [{complexity.upper()}] {db_name} timeout ({timeout//60}min): {gen_script}")
                self.stats['errors'].append(f"Generator timeout: {gen_script}")
            return False
        except Exception as e:
            with self.progress_lock:
                logger.warning(f"    ✗ [{complexity.upper()}] {db_name} error: {e}")
                self.stats['errors'].append(f"Generator {gen_script}: {str(e)[:100]}")
            return False
    ...
    def generate_dependent_data(self):
        ...
        logger.info(f"  📊 Generator distribution: {len(heavy_generators)} heavy, {len(medium_generators)} medium, {len(light_generators)} light")
        ...
        if heavy_generators:
            ...
            logger.info(f"  🏗️  Phase 1: Processing {len(heavy_generators)} heavy generators (dependency-ordered, {workers} workers)...")
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_gen):
                    ...
                    try:
                        ...
                        with self.progress_lock:
                            logger.warning(f"  ✗ Heavy generator exception for {db_name}: {exc}")
                            self.stats['errors'].append(f"Heavy generator exception {db_name}: {str(exc)[:100]}")
                    
                    with self.progress_lock:
                        logger.info(f"  📈 Progress: {completed}/{total_generators} ({completed/total_generators*100:.1f}%) - Heavy phase")
        ...
        if medium_generators:
            ...
            logger.info(f"  ⚙️  Phase 2: Processing {len(medium_generators)} medium generators (dependency-ordered, {workers} workers)...")
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_gen):
                    ...
                    try:
                        ...
                        with self.progress_lock:
                            logger.warning(f"  ✗ Medium generator exception for {db_name}: {exc}")
                            self.stats['errors'].append(f"Medium generator exception {db_name}: {str(exc)[:100]}")
                    
                    with self.progress_lock:
                        logger.info(f"  📈 Progress: {completed}/{total_generators} ({completed/total_generators*100:.1f}%) - Medium phase")
        ...
        if light_generators:
            ...
            logger.info(f"  ⚡ Phase 3: Processing {len(light_generators)} light generators (dependency-ordered, {workers} workers)...")
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_gen):
                    ...
                    try:
                        ...
                        with self.progress_lock:
                            logger.warning(f"  ✗ Light generator exception for {db_name}: {exc}")
                            self.stats['errors'].append(f"Light generator exception {db_name}: {str(exc)[:100]}")
                    
                    with self.progress_lock:
                        logger.info(f"  📈 Progress: {completed}/{total_generators} ({completed/total_generators*100:.1f}%) - Light phase")
        
        logger.info(f"\n✓ Generated data for {self.stats['data_generated']} databases using smart scheduling")
        return True
    ...
    def validate_single_database(self, db_name, config, registry):
        ...
        try:
            ...
            if errors:
                with self.progress_lock:
                    logger.warning(f"  ✗ {db_name}: {len(errors)} FK validation errors ({elapsed:.1f}s)")
                ...
                with self.progress_lock:
                    logger.info(f"  ✓ {db_name}: All FKs valid ({elapsed:.1f}s, {len(data)} tables)")
    ...
    def validate_referential_integrity(self):
        ...
        if not databases_to_validate:
            logger.info("  ⚠ No databases with data files found for validation")
            return True
        
        logger.info(f"  📊 Validating {len(databases_to_validate)} databases in parallel...")
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_db):
                ...
                try:
                    ...
                    with self.progress_lock:
                        logger.warning(f"  ✗ Validation exception for {db_name}: {exc}")
                        self.stats['errors'].append(f"Validation exception {db_name}: {str(exc)[:100]}")
                    total_errors += 1
                
                with self.progress_lock:
                    logger.info(f"  📈 Validation progress: {completed}/{len(databases_to_validate)} ({completed/len(databases_to_validate)*100:.1f}%)")
        
        if total_errors == 0:
            logger.info(f"\n✓ Parallel referential integrity validation passed")
            return True
        else:
            logger.warning(f"\n⚠ {total_errors} FK validation errors found across {len(databases_to_validate)} databases")
            return False
    ...
    def load_table_ultra_fast(self, cursor, table_name, records):
        ...
        try:
            ...
            if not safe_cols:
                logger.warning(f"    ⚠ No matching columns found for {table_name}, using all JSON columns")
                safe_cols = cols
            
        except Exception as e:
            logger.warning(f"    ⚠ Error getting DB column info for {table_name}: {str(e)[:100]}")
            safe_cols = cols
            db_col_info = {}
        ...
        try:
            ...
            logger.error(f"    ✗ COPY Error in {table_name}: {error_msg}")
    ...
    def load_table_worker(self, args):
        ...
        try:
            ...
            with self.progress_lock:
                logger.info(f"    ✓ {db_name}.{table_name}: {loaded} records")
            ...
            if len(error_msg) > 200:
                error_msg = error_msg[:200]
            with self.progress_lock:
                logger.error(f"    ✗ {db_name}.{table_name}: {error_msg}")
    ...
    def load_single_database(self, db_name, config):
        ...
        try:
            ...
            if not table_tasks:
                with self.progress_lock:
                    logger.info(f"  ⊘ No tables to load for {db_name}")
                return True
                ...
                with ThreadPoolExecutor(max_workers=max_table_workers) as executor:
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            with self.progress_lock:
                                logger.error(f"    ✗ {db_name}.{table_name} worker failed: {str(e)[:100]}")
            else:
            ...
            elapsed = time.time() - start_time
            with self.progress_lock:
                logger.info(f"  → {db_name}: {len(table_tasks)} tables, {total_loaded} records ({elapsed:.1f}s)")
            ...
            with self.progress_lock:
                logger.error(f"  ✗ {db_name}: {str(e)[:100]}")
    ...
    def load_data(self):
        ...
        logger.info(f"  📊 Loading strategy: {len(tier1_dbs)} base + {len(tier2_dbs)} core + {len(tier3_dbs)} extended databases")
        
        # Tier 1: Load foundational data first (2 workers)
        if tier1_dbs:
            logger.info(f"  🏗️  Tier 1: Loading {len(tier1_dbs)} foundational databases...")
            with ThreadPoolExecutor(max_workers=2) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_db):
                    ...
                    try:
                        ...
                        with self.progress_lock:
                            logger.warning(f"  ✗ Tier 1 loading exception for {db_name}: {exc}")
                            self.stats['errors'].append(f"Load exception {db_name}: {str(exc)[:100]}")
                    
                    with self.progress_lock:
                        logger.info(f"  📈 Progress: {completed}/{total_databases} ({completed/total_databases*100:.1f}%) - Tier 1")
        
        # Tier 2: Load core systems (3-4 workers)
        if tier2_dbs:
            workers = min(4, len(tier2_dbs))
            logger.info(f"  ⚙️  Tier 2: Loading {len(tier2_dbs)} core databases ({workers} workers)...")
            with ThreadPoolExecutor(max_workers=workers) as executor:
                future_to_db = {executor.submit(self.load_single_database, db_name, config): db_name 
                ...
                for future in concurrent.futures.as_completed(future_to_db):
                    ...
                    try:
                        ...
                        with self.progress_lock:
                            logger.warning(f"  ✗ Tier 2 loading exception for {db_name}: {exc}")
                            self.stats['errors'].append(f"Load exception {db_name}: {str(exc)[:100]}")
                    
                    with self.progress_lock:
                        logger.info(f"  📈 Progress: {completed}/{total_databases} ({completed/total_databases*100:.1f}%) - Tier 2")
        
        # Tier 3: Load remaining databases rapidly (5-6 workers)
        if tier3_dbs:
            workers = min(6, len(tier3_dbs))
            logger.info(f"  ⚡ Tier 3: Loading {len(tier3_dbs)} extended databases ({workers} workers)...")
            with ThreadPoolExecutor(max_workers=workers) as executor:
                future_to_db = {executor.submit(self.load_single_database, db_name, config): db_name 
                ...
                for future in concurrent.futures.as_completed(future_to_db):
                    ...
                    try:
                        ...
                        with self.progress_lock:
                            logger.warning(f"  ✗ Tier 3 loading exception for {db_name}: {exc}")
                            self.stats['errors'].append(f"Load exception {db_name}: {str(exc)[:100]}")
                    
                    with self.progress_lock:
                        logger.info(f"  📈 Progress: {completed}/{total_databases} ({completed/total_databases*100:.1f}%) - Tier 3")
        
        logger.info(f"\n✓ Dependency-aware parallel loading completed: {self.stats['tables_loaded']} tables, {self.stats['records_loaded']:,} records")
    ...
    def reset_sequences(self):
        ...
        if not databases_to_reset:
            logger.info("  ⚠ No databases found for sequence reset")
            return
        
        logger.info(f"  📊 Resetting sequences for {len(databases_to_reset)} databases in parallel...")
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_db):
                ...
                try:
                    ...
                    with self.progress_lock:
                        logger.warning(f"  ✗ Sequence reset exception for {db_name}: {exc}")
                
                with self.progress_lock:
                    logger.info(f"  📈 Sequence reset progress: {completed}/{len(databases_to_reset)} ({completed/len(databases_to_reset)*100:.1f}%)")
        
        logger.info(f"\n✓ Parallel sequence reset completed: {total_sequences} sequences reset across {len(databases_to_reset)} databases")
        ...
        try:
            ...
            for schema, seq_name in sequences:
                try:
                    ...
                    with self.progress_lock:
                        logger.warning(f"  ⚠ Failed to reset sequence {seq_name} in {db_name}: {str(e)[:80]}")
            ...
            with self.progress_lock:
                logger.info(f"  ✓ {db_name}: Reset {sequences_reset} sequences")
            
            return True, sequences_reset
            
        except Exception as e:
            with self.progress_lock:
                logger.error(f"  ✗ Sequence reset failed for {db_name}: {e}")
            return False, 0
